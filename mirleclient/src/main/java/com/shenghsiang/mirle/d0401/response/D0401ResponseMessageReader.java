...
public class D0401ResponseMessageReader implements MessageBodyReader<INDEX> {
	final static Logger logger = LoggerFactory.getLogger(D0401ResponseMessageReader.class);
		...
		try {
			...
			while (line != null) {				
				if (line.trim().length() != 0) {
					/*
					 * This is a bug from Mirle where the returned response is in Big5 format not in UTF-8
					 */
					logger.debug(line);
